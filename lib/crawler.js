...
var defaultLinkSelector = require('./linkSelectors/default')();
...
var Crawler = function(options) {
  ...
  this._linkSelector = defaultLinkSelector;
...
Crawler.prototype.crawl = function() {
  ...
  async.until(
    function(){
        if (self.options.maxPages) {
          return ((self._urlFrontier.emptyQueue()) || (self.stats.pagesCrawled > self.options.maxPages));
        } else {
          return self._urlFrontier.emptyQueue();
        }
    },
    function(done) {
...
Crawler.prototype.linkSelector = function(linkSelector) {
...
Crawler.prototype._parse = function(currentLink, response, done) {
  ...
  if (links) {
      var prettyLinks = JSON.stringify(links.map(function(link) {return link.href}), null, '  >> ');
      ...
      var self = this;
      this._linkSelector(links, context,
          function(selectedLinks, err) {
              if (err) {
                  done(err)
              } else {
                  // Default link handling
                  _.each(selectedLinks, function(href) {
                      self._urlFrontier.addUrl(currentLink, href);
                  }, self);

                  self.emit('links', selectedLinks);
                  done(null, item);
              }
          });
      } else {
